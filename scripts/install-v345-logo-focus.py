"""Install MOVX v346 Physical Logo visibility pass after shared model framing.
...
out=root/'_site'
release='v346-physical-logo-visible'
css='v345-logo-focus.css'
js='v345-logo-focus.mjs'

for name in (css,js):
    src=root/'site'/name
    if not src.exists():raise SystemExit(f'MOVX v346 missing source: {src}')
...
for name in ('index.html','latest.html'):
    ...
    if 'data-model-framing="v324-model-framing"' not in text:
        raise SystemExit(f'MOVX v346 expected v324 framing before logo focus: {name}')
...
print(json.dumps({'release':release,'scope':'hero-movx-logo only','stage':1,'goal':'real GLB visibly replaces fallback','css':css,'runtime':js,'pages':installed}))
