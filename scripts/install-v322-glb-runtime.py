"""Install MOVX v347 single-model GLB runtime after v321 production promotion.

Copies Three.js GLTFLoader and its relative addon dependencies from the pinned
npm package, then injects the runtime with a fresh cache key. Production WebGL
is gated by the runtime to the Physical Logo slot only.
"""
...
for name in (css,runtime):
    src=root/'site'/name
    dst=out/name
    if not src.exists():raise SystemExit(f'MOVX v347 missing source: {src}')
    shutil.copy2(src,dst)

three_dst=out/'vendor'/'three.module.js'
if not three_dst.exists():raise SystemExit('MOVX v347 requires local vendor/three.module.js from v136')
addons_src=root/'node_modules'/'three'/'examples'/'jsm'
entry=addons_src/'loaders'/'GLTFLoader.js'
if not entry.exists():raise SystemExit('MOVX v347 could not find pinned GLTFLoader.js')
...
def copy_module(src):
    ...
    if not inside(src,addons_src.resolve()):raise SystemExit(f'MOVX v347 addon escaped source root: {src}')
    if not src.exists():raise SystemExit(f'MOVX v347 addon dependency missing: {src}')
    visited.add(src)
...
if not loader_dst.exists():raise SystemExit('MOVX v347 loader copy failed')
if re.search(r"from\s+['\"]three['\"]",loader_dst.read_text()):raise SystemExit('MOVX v347 bare Three import survived')
...
for name in ('index.html','latest.html'):
    ...
    if 'data-movx-production="v321-production-storyboard"' not in text:
        raise SystemExit(f'MOVX v347 expected v321 production page: {name}')
...
fixture={
 'asset':{'version':'2.0','generator':'MOVX v347 QA'},
...
print(json.dumps({
 ...
 'fixture':'qa-v322-model.gltf','mode':'Physical Logo only in production'
}))
