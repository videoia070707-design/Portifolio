...
def ensure_media():
    ...
    with tempfile.TemporaryDirectory(prefix='movx-v127-') as tmpdir:
        src=Path(tmpdir)/'0923-source.mp4'
        req=urllib.request.Request(source_url,headers={'User-Agent':'MOVX-v127-builder/1.0'})
        with urllib.request.urlopen(req,timeout=90) as response, src.open('wb') as target:
            shutil.copyfileobj(response,target)
        if src.stat().st_size<1024:
            raise SystemExit('MOVX v127 source video download was empty')
    ...
    if not media_is_optimized():
        ...
        raise SystemExit(f'MOVX v127 media exceeds performance budget: {sizes}')
...
def run_base_build_without_retired_v116():
    ...
    with tempfile.TemporaryDirectory(prefix='movx-v127-retired-') as tmpdir:
        retired=Path(tmpdir)
        moved=[]
...
def optimize_hero():
    ...
    if not ffmpeg or not source.exists():
        raise SystemExit('MOVX v127 hero optimizer prerequisites are missing')
    ...
    if not target.exists() or target.stat().st_size>220_000:
        raise SystemExit(f'MOVX v127 optimized hero is too large: {target.stat().st_size if target.exists() else 0}')
...
def optimize_runtime():
    ...
    if text.count(eager)!=1:
        raise SystemExit('MOVX v127 expected one eager conveyor image template')
    text=text.replace(eager,lazy,1)
    warm="rootMargin:'900px 0px'"
    if text.count(warm)!=1:
        raise SystemExit('MOVX v127 expected one 900px archive predecode margin')
