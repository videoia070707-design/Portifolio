"""Build MOVX v125 true-3D Scroll World with trimmed, seek-friendly media."""
from pathlib import Path
import runpy, json, re, shutil, subprocess, tempfile, urllib.request

root=Path(__file__).resolve().parents[1]
out=root/'_site'
release='v125-true-3d-scroll'
...
def ensure_media():
    ...
    with tempfile.TemporaryDirectory(prefix='movx-v125-') as tmpdir:
        src=Path(tmpdir)/'0923-source.mp4'
        req=urllib.request.Request(source_url,headers={'User-Agent':'MOVX-v125-builder/1.0'})
        with urllib.request.urlopen(req,timeout=90) as response, src.open('wb') as target:
            shutil.copyfileobj(response,target)
        if src.stat().st_size<1024:
            raise SystemExit('MOVX v125 source video download was empty')
    ...
    if not media_is_optimized():
        ...
        raise SystemExit(f'MOVX v125 media exceeds performance budget: {sizes}')
...
def run_base_build_without_retired_v116():
    ...
    with tempfile.TemporaryDirectory(prefix='movx-v125-retired-') as tmpdir:
        retired=Path(tmpdir)
        moved=[]
...
def optimize_hero():
    ...
    if not ffmpeg or not source.exists():
        raise SystemExit('MOVX v125 hero optimizer prerequisites are missing')
    ...
    if not target.exists() or target.stat().st_size>220_000:
        raise SystemExit(f'MOVX v125 optimized hero is too large: {target.stat().st_size if target.exists() else 0}')
...
def optimize_runtime():
    ...
    if text.count(eager)!=1:
        raise SystemExit('MOVX v125 expected one eager conveyor image template')
    text=text.replace(eager,lazy,1)
    warm="rootMargin:'900px 0px'"
    if text.count(warm)!=1:
        raise SystemExit('MOVX v125 expected one 900px archive predecode margin')
...
if missing or not installed:raise SystemExit(f'v125 invalid build: missing={missing}; installed={installed}')
media_sizes={key:path.stat().st_size for key,path in media_files.items()}
print(json.dumps({'release':release,'pages':installed,'source':source_url,'source_trim_seconds':float(clip_start),'media_mode':media_mode,'hero':hero_stats,'media_bytes':media_sizes,'desktop':'camera-3d + trimmed H264-first deferred blob scrub','mobile':'camera-3d + trimmed H264-first deferred blob scrub','scroll_world':'true perspective camera path + real scroll-linked video + 4 chapters','missing':missing}))