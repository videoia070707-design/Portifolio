...
(async()=>{
  ...
  await page.waitForFunction(()=>document.documentElement.dataset.modelFramingRevision==='v336-mobile-logo-fit',{timeout:30000});
  ...
  await mobile.waitForFunction(()=>document.documentElement.dataset.modelFramingRevision==='v336-mobile-logo-fit',{timeout:30000});
  ...
  const worstYaw=.10; // conservative: full v331 yaw, larger than coarse-pointer amplitude
  const rendererScale=1.011; // max CSS logo-progress scale
  const mobileHostStretch=1.04; // v324 mobile inset 0 -2%
  ...
  const minCameraZ=mobileLogo.tuning.cameraZ-.06; // conservative: full v331 dolly-out endpoint
