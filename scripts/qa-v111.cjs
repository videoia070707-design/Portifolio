...
(async()=>{
  ...
  async function capture(name,fraction){
    ...
    const validCorridorClip=['49%','50%','52%'].some(v=>state.canvas.clip.includes(v));
    if(state.canvas.display==='none'||state.canvas.opacity<.45||!validCorridorClip)failures.push('3d-field-present');
