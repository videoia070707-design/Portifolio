...
(async()=>{
  ...
  async function capture(name,fraction){
    ...
    if(state.canvas.display==='none'||state.canvas.opacity<.54||state.canvas.opacity>.62)failures.push('canvas-opacity-owner');
    if(state.canvas.transform==='none'||!state.canvas.transform.includes('0.7'))failures.push('canvas-scale-owner');
    ...
    if(state.canvas.blend!=='screen')failures.push('canvas-blend');
  ...
  await browser.close();
  console.log('MOVX v114: final runtime ownership, clean copy lane, softened transparent WebGL field, scroll progression and mobile fallback validated.');
