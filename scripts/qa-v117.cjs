...
(async()=>{
  ...
  try{
    ...
    page.on('request',request=>{if(/movx-scroll-world-0923\.(?:mp4|webm)/.test(request.url()))mediaRequests.push(request.url())});
    ...
    assert.equal(mediaRequests.length,0,'Scroll World media must stay idle before the chapter reaches the viewport');
    ...
    assert.ok(/movx-scroll-world-0923\.(mp4|webm)(?:$|\?)/.test(desktopSource)||desktopSource.startsWith('blob:'),'desktop source must use the optimized media or blob fallback');
    ...
    assert.ok(/movx-scroll-world-0923-mobile\.(mp4|webm)(?:$|\?)/.test(mobileSource)||mobileSource.startsWith('blob:'),'mobile must use the smaller optimized source or blob fallback');
    ...
    await reduced.close();
    console.log(JSON.stringify({status:'passed',initialMediaRequests:mediaRequests.length,desktopSource,forward:[first,last],reverse,mobileSource,reduced:'poster'}));
