...
function makeSection(items){
  ...
  section.innerHTML=`<div class="v52-architecture-sticky">
    ...
    <div class="v52-fallback-stage" aria-hidden="true"><div class="v52-fallback-case"></div>${items.map(p=>`<img class="v52-fallback-art" src="${escapeHTML(p.cover)}" alt="">`).join('')}<div class="v52-fallback-rails"><i></i><i></i></div><div class="v52-fallback-gates"></div></div>
...
function initDomFallback(section,items){
  ...
  const stage=section.querySelector('.v52-fallback-stage');
  const caseEl=stage?.querySelector('.v52-fallback-case');
  const arts=[...(stage?.querySelectorAll('.v52-fallback-art')||[])];
  const rails=stage?.querySelector('.v52-fallback-rails');
  const gatesHost=stage?.querySelector('.v52-fallback-gates');
  const processRows=[...document.querySelectorAll('#process .process-list li')];
  if(!stage||!caseEl||!arts.length||!gatesHost) return;
  processRows.slice(0,5).forEach((row,i)=>{
    const gate=document.createElement('div');gate.className='v52-fallback-gate';
    gate.innerHTML=`<span>${String(i+1).padStart(2,'0')}</span><b>${escapeHTML(row.querySelector('strong')?.textContent?.trim()||`Etapa ${i+1}`)}</b>`;
    gatesHost.append(gate);
  });
  const gateEls=[...gatesHost.children];
  gsap.set(gateEls,{opacity:0,z:-180,scale:.82});
  if(rails) gsap.set(rails,{opacity:0,scaleY:.2});
  const tl=gsap.timeline({scrollTrigger:{trigger:section,start:'top top',end:'bottom bottom',scrub:1.05,invalidateOnRefresh:true}});
  tl.fromTo(caseEl,{x:210,y:120,z:-430,rotationX:-9,rotationY:22,rotationZ:-2,scale:.7,opacity:0},{x:130,y:22,z:0,rotationX:-5,rotationY:13,rotationZ:-1,scale:.94,opacity:1,duration:.19,ease:'none'},0)
    .to(caseEl,{x:70,y:-10,z:-65,rotationX:-2,rotationY:7,scale:.88,duration:.18,ease:'none'},.52)
    .to(caseEl,{x:0,y:0,z:-210,rotationX:0,rotationY:0,scaleX:1.35,scaleY:.42,opacity:.18,duration:.16,ease:'none'},.66)
    .to(caseEl,{z:-410,opacity:0,duration:.12,ease:'none'},.86);
  arts.forEach((art,i)=>{
    const side=i%2?-1:1,rank=i-(arts.length-1)/2,start=.20+i*.052;
    const fanX=side*(220+Math.abs(rank)*54)+70;
    const fanY=rank*74+18;
    const corridorX=side*(72+Math.abs(rank)*15);
    const corridorY=rank*30;
    tl.fromTo(art,{x:140,y:34,z:-190,rotationX:7,rotationY:side*3,rotationZ:rank*2.2,scale:.48,opacity:0},{x:fanX,y:fanY,z:100+Math.abs(rank)*22,rotationX:-2,rotationY:side*(-10-rank),rotationZ:rank*3.1,scale:.9,opacity:.96,duration:.34,ease:'none'},start)
      .to(art,{x:corridorX,y:corridorY,z:-20-i*135,rotationX:-1,rotationY:side*3.5,rotationZ:0,scale:.72,opacity:.82,duration:.18,ease:'none'},.67)
      .to(art,{z:-390-i*75,scale:.58,opacity:0,duration:.12,ease:'none'},.89);
  });
  if(rails) tl.to(rails,{opacity:.45,scaleY:1,duration:.14,ease:'none'},.64).to(rails,{opacity:0,duration:.1,ease:'none'},.91);
  gateEls.forEach((gate,i)=>{
    tl.to(gate,{opacity:.72,z:-i*90,scale:1,duration:.12,ease:'none'},.67+i*.035)
      .to(gate,{opacity:i===0?.88:.52,duration:.06,ease:'none'},.78+i*.025)
      .to(gate,{opacity:0,z:-260-i*80,duration:.1,ease:'none'},.91);
  });
  ScrollTrigger.create({trigger:section,start:'top top',end:'bottom bottom',onUpdate:self=>{
    section.style.setProperty('--v52-progress',String(self.progress));
    const idx=Math.min(items.length-1,Math.max(0,Math.floor(clamp((self.progress-.23)/.38)*items.length)));
    ...
    section.style.setProperty('--v52-caption-o',String(clamp((self.progress-.22)/.16)*clamp((.68-self.progress)/.12)));
  }});
}
...
function createBendGeometry(){const g=new THREE.PlaneGeometry(1,1,22,16);g.userData.base=new Float32Array(g.attributes.position.array);return g;}
function applyBend(mesh,bend,twist=0){
  ...
  for(let i=0;i<pos.count;i++){
    ...
    const curve=(x*x-.25)*bend,torsion=x*y*twist,flutter=Math.sin((y+.5)*Math.PI)*x*bend*.08;
...
function makeArtwork(scene,project,index){
  ...
  new THREE.TextureLoader().load(project.cover,tex=>{
    ...
    const img=tex.image||{},aspect=(img.width&&img.height)?img.width/img.height:1,h=2.58,w=Math.min(4.0,h*aspect);
...
function makeGate(scene,label,index){
  ...
  const pts=[-2.65,-1.55,0,2.65,-1.55,0,2.65,-1.55,0,2.65,1.55,0,2.65,1.55,0,-2.65,1.55,0,-2.65,1.55,0,-2.65,-1.55,0];
  ...
  const textMat=new THREE.MeshBasicMaterial({map:tex,color:root.getAttribute('data-theme')==='dark'?0xf1eae4:0x2c2724,transparent:true,opacity:0,depthWrite:false});
...
function init(){
  ...
  [-2.82,2.82].forEach(x=>{const geom=new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(x,-1.72,2.2),new THREE.Vector3(x,1.72,-15.8)]);const line=new THREE.Line(geom,railMat);scene.add(line);rails.push(line);});

  let target=0,current=0,vel=0,velTarget=0,active=false,raf=0,contextLost=false,currentCaption=-1,currentPhase='',painted=false;
  ...
  function setCaption(index,opacity){if(index!==currentCaption&&items[index]){currentCaption=index;caption.textContent=`${items[index].client} — ${items[index].title}`;}section.style.setProperty('--v52-caption-o',String(opacity));section.style.setProperty('--v52-caption-y',`${lerp(10,0,opacity)}px`);}
  function setPhase(text,t){if(text!==currentPhase){currentPhase=text;phase.textContent=text;}section.style.setProperty('--v52-phase-line',String(lerp(.18,1,t)));}
  function setProcessCurrent(index){processRows.forEach((row,i)=>row.classList.toggle('v52-process-current',i===index));}
  function deconstructBox(t,exit,open){
    const ease=smooth(t);
    const move=(obj,toP,toR,toS=1)=>{const a=box.initial.get(obj);if(!a)return;obj.position.lerpVectors(a.p,toP,ease);obj.rotation.set(lerp(a.r.x,toR.x,ease),lerp(a.r.y,toR.y,ease),lerp(a.r.z,toR.z,ease));const s=lerp(1,toS,ease)*(1-exit*.18);obj.scale.setScalar(s);};
    ...
    const lidBase=box.initial.get(box.lidPivot);
    if(lidBase){box.lidPivot.position.lerpVectors(lidBase.p,new THREE.Vector3(0,1.8,-6.2),ease);box.lidPivot.rotation.set(lerp(lerp(0,-1.42,open),-1.08,ease),0,0);box.lidPivot.scale.setScalar(lerp(1,.9,ease)*(1-exit*.18));}
    box.materials.forEach(m=>m.opacity=clamp(1-exit*.85));
    box.logo.material.opacity=clamp(.9*(1-ease*.68-exit));
  }

  function update(p){
    const rise=smooth(clamp(p/.15)),open=smooth(clamp((p-.09)/.20)),fan=smooth(clamp((p-.23)/.36)),deconstruct=smooth(clamp((p-.60)/.18)),corridorT=smooth(clamp((p-.68)/.18)),processT=smooth(clamp((p-.76)/.16)),exit=smooth(clamp((p-.91)/.09));
    box.group.position.y=lerp(-1.0,.08,rise)-exit*.4;box.group.position.z=lerp(-4.0,.08,rise)-deconstruct*.35-exit*2.6;box.group.rotation.y=.22+pointer.x*.055+vel*.025;box.group.rotation.x=-.07+pointer.y*.03;box.group.scale.setScalar(lerp(.74,.92,rise)*(1-exit*.12));
    box.lidPivot.rotation.x=lerp(0,-1.42,open);deconstructBox(deconstruct,exit,open);
    let nearest=0,nearestDist=9;
    arts.forEach((g,i)=>{
      const start=.21+i*.06,end=start+.32,e=smooth(clamp((p-start)/(end-start))),side=i%2?-1:1,rank=i-1.5;
      const fanX=side*(1.45+Math.abs(rank)*.46)+1.0,fanY=rank*.48+e*.18,fanZ=2.65+Math.abs(rank)*.28,corridorX=side*(.76+Math.abs(rank)*.12),corridorY=rank*.15,corridorZ=1.4-i*3.05;
      g.userData.mat.opacity=clamp(e*(1-exit*.97));g.userData.frameMat.opacity=clamp(1-exit*.75);
      g.position.x=lerp(lerp(1.45,fanX,e),corridorX,corridorT);g.position.y=lerp(lerp(-.14,fanY,e),corridorY,corridorT)-exit*.25;g.position.z=lerp(lerp(.65,fanZ,e),corridorZ,corridorT)-exit*2.2;
      g.rotation.y=lerp(lerp(0,side*(-.17-rank*.02),e),side*.055,corridorT)+pointer.x*.025;g.rotation.x=lerp(pointer.y*-.02+vel*.012,-.015,corridorT);g.rotation.z=lerp(lerp((i-1.5)*.018,rank*.042,e),0,corridorT);g.scale.setScalar(lerp(.48,.96,e)*(1-exit*.12));
      const bend=(1-corridorT)*(0.095*(1-e)+Math.abs(vel)*.07)+corridorT*.018,twist=(1-corridorT)*(side*.055*e+vel*.018);applyBend(g.userData.plane,bend,twist);
    ...
    gates.forEach((g,i)=>{const local=smooth(clamp((corridorT-(i*.055))/Math.max(.001,1-i*.055))),focus=clamp(1-Math.abs(processT-i/Math.max(1,gates.length-1))/.27,0,1);g.userData.lineMat.opacity=(.025+local*.13+focus*.12)*(1-exit);g.userData.accentMat.opacity=(.02+local*.16+focus*.18)*(1-exit);g.userData.textMat.opacity=(.018+local*.10+focus*.22)*(1-exit);g.scale.setScalar(.94+focus*.08);});
    ...
    section.style.setProperty('--v52-progress',String(clamp(p)));root.style.setProperty('--v52-journey-opacity',String(lerp(.05,.76,processT)*(1-exit*.15)));
    ...
    camera.position.x=pointer.x*.22+vel*.07;camera.position.y=-pointer.y*.13;camera.position.z=11.7-corridorT*.9-processT*.25;camera.fov=42+Math.min(4.2,Math.abs(vel)*3);camera.updateProjectionMatrix();camera.lookAt(lerp(.45,0,corridorT),lerp(.05,-.12,processT),lerp(.1,-3.8,processT));
  }
  ...
  function paint(){
    ...
    if(!painted){painted=true;root.classList.add('v52-webgl-painted');}
  ...
  const trigger=ScrollTrigger.create({
    ...
    onUpdate(self){if(self.isActive&&!active){active=true;root.classList.add('v52-architecture-active');resize();}target=self.progress;velTarget=clamp(self.getVelocity()/2200,-1.2,1.2);schedule();}
  });
  ...
  canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();contextLost=true;root.classList.remove('v52-webgl-painted');root.classList.add('v52-fallback');setProcessCurrent(-1);},{once:true});
  let resizeRaf=0;
  ...
  new MutationObserver(muts=>{if(muts.some(m=>m.attributeName==='data-theme')){scene.fog.color.setHex(root.getAttribute('data-theme')==='dark'?0x0b0909:0xf1ece7);gates.forEach(g=>g.userData.textMat.color.setHex(root.getAttribute('data-theme')==='dark'?0xf1eae4:0x2c2724));schedule();}}).observe(root,{attributes:true,attributeFilter:['data-theme']});
